<template>
  <div class="component h-100" :style="{'background-image': 'url(' + require('../../assets/sfondo.jpg') + ')'}">
    <div class="col-xs-12 col-sm-12 col-md-4 mx-auto mt-3">
      <div v-if="!loading">
        <br/>
        ...
        <b-card class="card mt-5">
          <b-card-text>
            <div class="titleCard">
              <p>Miglior giocatore</p>
            </div>
            <b-table :small=true class="mt-3" striped hover :items="bests" :fields="fields">
              <template slot="name" slot-scope="data" >
                {{data.item[0]}} {{data.item[1]}}
              </template>
              <template slot="best" slot-scope="data" >
                {{data.item[2]}}
              </template>
            </b-table>
          </b-card-text>
